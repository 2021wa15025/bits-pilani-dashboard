...
export function CourseCard({ course, viewMode, onCourseClick }: CourseCardProps) {
  ...
  return (
    <Card className="group professional-card cursor-pointer hover:shadow-lg transition-all w-full flex flex-col h-full p-2 sm:p-3" onClick={() => onCourseClick?.(course)}>
      <div className="h-24 sm:h-32 rounded-t-lg overflow-hidden relative">
        <ImageWithFallback 
          src={courseImage} 
          alt={course.title}
  ...
  <div className="p-2 sm:p-3 flex flex-col flex-1">
        ...
        <div className="mb-2 sm:mb-3">
          <h3 className="font-semibold text-university-primary mb-1 text-xs sm:text-sm leading-normal min-h-[2rem] flex items-start">{course.title}</h3>
          <p className="text-university-secondary text-[11px] sm:text-xs">{course.code} • Semester {course.semester}</p>
        </div>
