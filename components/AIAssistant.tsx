    ...
    if (isCourseQuery) {
      if (lowerMessage.includes('ongoing') || lowerMessage.includes('current')) {
        ...
        if (ongoingCourses.length === 1) {
          ...
          const encouragement = progress >= 80 ? 'Great work! 🎉' : progress >= 60 ? 'You\'re doing well! 👍' : 'Keep it up! 💪';
          
          return `You're currently taking **${course.title}** (${course.code}).

Your progress is at ${progress}%. ${encouragement}
                ...
                <div className="flex items-center justify-between relative z-10">
                  <div className="flex items-center gap-2 sm:gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs sm:text-sm opacity-90 text-white/80">AI Assistant</p>
                    </div>
                  </div>
                  <Button
                    variant="ghost"
