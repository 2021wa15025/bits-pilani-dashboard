import image_7f13a12b6e7cf3d6a516fe74feab5ad5b5053146 from '../src/assets/headerLogo';
import { Bell, Search, LogOut, User, Settings, Sun, Moon, BookOpen, FileText, Calendar, Download, X, Menu, Megaphone, Edit, MessageCircle } from "lucide-react";
...
export function Header({ 
  ...
  const uniqueAnnouncements = Array.from(
    new Map(announcements.map(item => [item.id, item])).values()
  );

  const unreadAnnouncementsCount = uniqueAnnouncements.filter(a => !a.read).length;
  const unreadNotificationsCount = uniqueNotifications.filter(n => !n.read).length;
  ...
  const performSearch = (query: string) => {
    ...
    uniqueAnnouncements.forEach(announcement => {
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      case 'course': return 'bg-[#191f5e] text-white';
  ...
  return (
    <header className="university-header flex items-center justify-between w-full px-2 xs:px-3 sm:px-6 py-2 xs:py-3">
      {/* Mobile Menu Button & Logo */}
      <div className="flex items-center gap-1 xs:gap-2 sm:gap-4 min-w-0 flex-shrink-0">
        {/* Mobile Menu Button - Only show on mobile */}
        <button
          onClick={onMobileMenuToggle}
          className="lg:hidden text-white hover:bg-white/10 p-1 xs:p-2 min-w-0 w-8 h-8 xs:w-9 xs:h-9 rounded-md transition-colors flex items-center justify-center"
        >
          <Menu className="w-4 h-4 xs:w-5 xs:h-5 text-white" />
        </button>
      ...
      <div className="hidden sm:flex flex-1 max-w-xl mx-2 lg:mx-8" ref={searchRef}>
        <div className="relative w-full">
          <Input 
            ...
            className="form-input border-slate-200 bg-slate-50 text-slate-900 placeholder-slate-500 focus:bg-white focus:text-slate-900 focus:placeholder-slate-500 dark:border-slate-600 dark:bg-slate-700 dark:text-white dark:placeholder-slate-300 dark:focus:bg-slate-600 dark:focus:text-white dark:focus:placeholder-slate-200 w-full max-w-80 px-4 py-3"
          />
          {searchQuery && (
            <button
              onClick={clearSearch}
              className="absolute right-1 top-1/2 transform -translate-y-1/2 h-8 w-8 p-0 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-500 dark:text-slate-300 rounded-md transition-colors flex items-center justify-center"
            >
      ...
      <div className="flex items-center gap-1 xs:gap-2 sm:gap-4 min-w-0 flex-shrink-0">
        ...
        <button
          ...
          className="sm:hidden text-slate-600 hover:bg-slate-100 p-1 xs:p-2 min-w-0 w-8 h-8 xs:w-9 xs:h-9 rounded-md transition-colors flex items-center justify-center"
        >
        ...
        >
          {theme === "light" ? (
            <Moon className="w-4 h-4 xs:w-5 xs:h-5 text-white" />
          ) : (
            <Sun className="w-4 h-4 xs:w-5 xs:h-5 text-white" />
          )}
        </button>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              className="rounded-lg hover:bg-white/10 relative transition-colors text-white w-8 h-8 xs:w-9 xs:h-9 sm:w-10 sm:h-10 flex items-center justify-center"
            >
              <Bell className="w-4 h-4 xs:w-5 xs:h-5 text-white" />
              {unreadNotificationsCount > 0 && (
                <Badge className="absolute -top-1 -right-1 w-4 h-4 xs:w-5 xs:h-5 p-0 bg-red-500 text-white text-xs flex items-center justify-center animate-pulse min-w-0">
                  <span className="text-xs leading-none">{unreadNotificationsCount > 9 ? '9+' : unreadNotificationsCount}</span>
                </Badge>
              )}
            </button>
          </DropdownMenuTrigger>
          ...
          >
            <div className="p-2">
              <h3 className="font-semibold text-sm text-[#191f5e] dark:text-blue-400 mb-2">System Notifications</h3>
              {uniqueNotifications.length === 0 ? (
                ...
                uniqueNotifications.map((notification) => (
                  <div
                    key={notification.id}
                    className={`p-3 rounded-lg mb-2 cursor-pointer transition-colors hover:bg-gray-50 dark:hover:bg-slate-700/50 ${
                      notification.read 
                        ? 'bg-gray-50/50 dark:bg-slate-700/30' 
                        : 'bg-green-50 dark:bg-green-500/10 border-l-4 border-green-500 dark:border-green-400'
                    }`}
                    onClick={() => onNotificationClick?.(notification.id)}
                  >
                    <div className="flex items-center justify-between mb-1">
                      <h4 className="font-medium text-sm text-gray-900 dark:!text-white">{notification.title}</h4>
                      <span className="text-xs px-2 py-1 rounded bg-green-100 dark:bg-green-500/20 text-green-700 dark:!text-white">
                        System
                      </span>
                    </div>
                    <p className="text-xs text-gray-600 dark:!text-white mt-1 line-clamp-2">{notification.message}</p>
                    <p className="text-xs text-gray-400 dark:!text-white/70 mt-1">{notification.time}</p>
                  </div>
                ))
              )}
            </div>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              className="rounded-lg hover:bg-white/10 relative transition-colors text-white w-8 h-8 xs:w-9 xs:h-9 sm:w-10 sm:h-10 flex items-center justify-center"
            >
              <Megaphone className="w-4 h-4 xs:w-5 xs:h-5 text-white" />
              {unreadAnnouncementsCount > 0 && (
                <Badge className="absolute -top-1 -right-1 w-4 h-4 xs:w-5 xs:h-5 p-0 bg-red-500 text-white text-xs flex items-center justify-center animate-pulse min-w-0">
                  <span className="text-xs leading-none">{unreadAnnouncementsCount > 9 ? '9+' : unreadAnnouncementsCount}</span>
                </Badge>
              )}
            </button>
          </DropdownMenuTrigger>
          ...
          >
            <div className="p-2">
              <h3 className="font-semibold text-sm text-[#191f5e] dark:text-blue-400 mb-2">Announcements</h3>
              {uniqueAnnouncements.length === 0 ? (
                <p className="text-sm text-gray-500 dark:text-slate-400 text-center py-4">No announcements</p>
              ) : (
                  ...
                  <div
                    ...
                    className={`p-3 rounded-lg mb-2 cursor-pointer transition-colors hover:bg-gray-50 dark:hover:bg-slate-700/50 ${
                      announcement.read 
                        ...
                        ? 'bg-orange-50 dark:bg-orange-500/10 border-l-4 border-orange-500 dark:border-orange-400'
                        : 'bg-blue-50 dark:bg-blue-500/10 border-l-4 border-[#191f5e] dark:border-blue-400'
                    }`}
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className={`text-xs px-2 py-1 rounded ${
                        announcement.priority === 'high' ? 'bg-red-100 dark:bg-red-500/20 text-red-700 dark:!text-white' :
                        announcement.priority === 'medium' ? 'bg-orange-100 dark:bg-orange-500/20 text-orange-700 dark:!text-white' :
                        'bg-blue-100 dark:bg-blue-500/20 text-blue-700 dark:!text-white'
                      }`}>
                        {announcement.category}
                      </span>
                    </div>
                    <p className="text-xs text-gray-600 dark:!text-white mt-1 line-clamp-2">{announcement.content}</p>
                    <p className="text-xs text-gray-400 dark:!text-white/70 mt-1">{announcement.time}</p>
                  </div>
                ))
              )}
            </div>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <div className="flex items-center gap-1 xs:gap-2 sm:gap-3 hover:bg-white/10 rounded-lg p-1 xs:p-2 cursor-pointer transition-colors min-w-0">
              <Avatar className="w-7 h-7 xs:w-8 xs:h-8 sm:w-10 sm:h-10 ring-1 xs:ring-2 ring-white/30 flex-shrink-0">
                ...
                <AvatarFallback className="bg-[#191f5e] text-white font-semibold text-xs xs:text-sm">
          ...
          <DropdownMenuContent align="end" className="w-48 bg-white border border-gray-200 shadow-lg">
            <div className="p-1">
              <DropdownMenuItem 
                className="cursor-pointer text-[#191f5e] hover:bg-blue-50" 
